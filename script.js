...
function openTapeTheater(idx) {
    ...
    // ==========================================
    // 📝 1. อัปเดตข้อมูลส่วน Header
    // ==========================================
    document.getElementById('cardTapeTitle').innerText = tape.lecture_title || "ไม่ระบุหัวข้อ";
    document.getElementById('detailsTypeBadge').innerText = tape.type || "ไม่ระบุหมวดหมู่";

    // ==========================================
    // 🎙️ 2. อัปเดตข้อมูลการบรรยาย
    // ==========================================
    document.getElementById('detailsMeetTitle').innerText = tape.meet_title || "-";
    
    // จัดการแปลงรูปแบบวันที่ให้อ่านง่ายขึ้น (เป็นภาษาไทย)
    let formattedDate = "-";
    if (tape.date) {
        const d = new Date(tape.date);
        if (!isNaN(d.getTime())) {
            ...
            formattedDate = tape.date.split(" 00:00:00")[0] || tape.date; 
        }
    }
    document.getElementById('detailsLectureDate').innerText = formattedDate;

    document.getElementById('detailsLecturer').innerText = tape.lecturer || "-";
    document.getElementById('detailsLecturedAt').innerText = tape.lectured_at || "-";

    // ==========================================
    // 📜 3. อัปเดตข้อมูลธรรมนิพนธ์ (ซ่อน/แสดง อัตโนมัติ)
    // ==========================================
    const goshoSection = document.getElementById('goshoDetailsSection');
    if (tape.type === "ธรรมนิพนธ์") {
        goshoSection.style.display = "block"; 
        document.getElementById('detailsWrittenYear').innerText = tape.written_year || "-";
        document.getElementById('detailsWrittenAt').innerText = tape.written_at || "-";
        document.getElementById('detailsWrittenTo').innerText = tape.written_to || "-";
    } else {
        goshoSection.style.display = "none"; 
    }

    // ==========================================
    // 📥 4. จัดการปุ่มดาวน์โหลดเอกสาร
    // ==========================================
    const btnDownload = document.getElementById('btnDownloadDoc');
    const docLink = tape.document_link || tape.file_url || tape.download_url || ""; 
    if (docLink) {
        btnDownload.href = docLink;
    ...
    // ==========================================
    // 🎨 6. จัดการ UI Sidebar Active State
    // ==========================================
    document.querySelectorAll('.tape-item').forEach(el => el.classList.remove('active'));
    if(document.getElementById(`t-node-${idx}`)) {
        document.getElementById(`t-node-${idx}`).classList.add('active');
    }
